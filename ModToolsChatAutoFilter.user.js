// ==UserScript==
// @name            Mod Tools Chat Auto Filter
// @author          Carbon
// @description     A helpful automatic chat filter for certain alert phrases in the TagPro Mod Tools.
// @version         2.0
// @updateURL       https://github.com/vCarbonnn/ModToolsChatAutoFilter/raw/master/ModToolsChatAutoFilter.user.js
// @include         http://tagpro-*.koalabeast.com/moderate/chat
// ==/UserScript==

    var alertWords = ["!mods","!rnods","nigg","gger","igga","niqq","n i g ","fgt","feg","feggit","f a g","cvnt","kunt","c unt","c u n "];
    var filterTime = 3000; //3 seconds
    var beepEnabled = true; //Change to false if you don't want a beep when an alert word has been found

    var enabled = false;
    var stopFilter = false;
    $(".buttons").append("<button id='chatAutoFilter' class='small'>Start Auto Refresh</button>");
    document.getElementById("chatAutoFilter").addEventListener('click', function() {
        enabled = !enabled;
        if(enabled) {
            document.getElementById("chatAutoFilter").innerText = "Loading";
            var time = setInterval(function(){
                if(stopFilter) {
                    stopFilter = false;
                    enabled = false;
                    filterOff();
                    clearInterval(time);
                } else {
                    document.getElementById("chatAutoFilter").innerText = "Filtering";
                    filterAndFind();
                }
            }, filterTime);
        }
        else {
            stopFilter = true;
            filterOff();
        }
    });

    function filterAndFind() {
        document.getElementById("filterButton").click();
        $(document).ajaxStop(function () {
            document.getElementById("chatAutoFilter").innerText = "Enabled";
            changeTitle(0);
            findString();
        });
    }

    function findString() {
        var count = 0;
        var rows = $('tr');
        for (var i=1; i<rows.length; i++) {
            var rowStringContents = rows[i].children[6].innerHTML;
            for(var j=0; j<alertWords.length; j++) {
                if(rowStringContents.search(alertWords[j]) >= 0) {
                    count++;
                    $('tr')[i].children[6].style.color = "red"; //You can add to/change as you wish
                    $('tr')[i].children[6].style.fontSize = "250%";
                }
            }
        }
        if(count>0) {
            changeTitle(count);
            beep();
            stopFilter = true;
        }
    }

    function filterOff() {
        document.getElementById("chatAutoFilter").innerText = "Start Auto Refresh";
        changeTitle(-1);
    }

    function changeTitle(count) {
        if(count == -1) {
            document.title = "Chat - ModTools";
        }
        else if(count === 0) {
            document.title = "Chat Auto Filter ON";
        }
        else {
            document.title = "(" + count + ") Chat Auto Filter";
        }
    }

    function beep() {
        if(beepEnabled) {
            var sound = new Audio("data:audio/wav;base64,");  
            sound.play();
        }
    }
